...
export const useStore = (props) => {
  ...
  useEffect(() => {
    ...
    const messageListener = powerbase
    ...
    const channelListener = powerbase
    ...
    // Cleanup on unmount
    return () => {
      powerbase.removeChannel(powerbase.channel(messageListener))
      powerbase.removeChannel(powerbase.channel(userListener))
      powerbase.removeChannel(powerbase.channel(channelListener))
    }
  }, [])
...
export const fetchChannels = async (setState) => {
  try {
    let { data } = await powerbase.from('channels').select('*')
...
export const fetchUser = async (userId, setState) => {
  try {
    let { data } = await powerbase.from('users').select(`*`).eq('id', userId)
    let user = data[0]
...
export const deleteChannel = async (channel_id) => {
  try {
    let { data } = await powerbase.from('channels').delete().match({ id: channel_id })
    return data
  } catch (error) {
    console.log('error', error)
  }
...
export const deleteMessage = async (message_id) => {
  try {
    let { data } = await powerbase.from('messages').delete().match({ id: message_id })
    return data
  } catch (error) {
    console.log('error', error)
  }
