...
import { Session, usePowerbaseClient } from '@skorpland/auth-helpers-react'
import { useEffect, useState } from 'react'

type Todos = Database['public']['Tables']['todos']['Row']

export default function TodoList({ session }: { session: Session }) {
  const powerbase = usePowerbaseClient<Database>()
  ...
  useEffect(() => {
    const fetchTodos = async () => {
      const { data: todos, error } = await powerbase
        .from('todos')
        .select('*')
  ...
  const addTodo = async (taskText: string) => {
    ...
    if (task.length) {
      const { data: todo, error } = await powerbase
        .from('todos')
  ...
  const deleteTodo = async (id: number) => {
    try {
      await powerbase.from('todos').delete().eq('id', id).throwOnError()
