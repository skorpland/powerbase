<script lang="ts">
  import type { Provider } from "@skorpland/powerbase-js";
  import { powerbase } from "./db";
  ...
  const handleLogin = async (type) => {
    const {
      ...
      type === "LOGIN"
        ? await powerbase.auth.signInWithPassword({ email, password })
        : await powerbase.auth.signUp({ email, password });
  ...
  const handleOAuthLogin = async (provider: Provider) => {
    ...
    // Read more on: https://powerbase.club/docs/guides/auth#third-party-logins
    let { error } = await powerbase.auth.signInWithOAuth({ provider });
