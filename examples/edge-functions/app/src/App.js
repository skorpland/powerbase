...
function App() {
  ...
  const [powerFunction, setPowerFunction] = useState(functionsList[0])
  const [requestJson, setRequestJson] = useState(sampleObject)
  const [responseJson, setResponseJson] = useState({})

  const invokeFunction = async () => {
    setResponseJson({ loading: true })
    const { data, error } = await powerbase.functions.invoke(powerFunction, {
  ...
  return (
    <div className="p-2">
      <h2 className="mb-2 text-4xl">Powerbase Egde Functions Test Client</h2>
      <div className="grid grid-cols-1 md:grid-cols-2">
        <div className="p-2">
          ...
          <select
            ...
            onChange={(e) => setPowerFunction(e.target.value)}
          >
          ...
          {user ? (
            <div>
              ...
              <button
                ...
                onClick={() => powerbase.auth.signOut()}
              >
                Sign out
              </button>
            </div>
          ) : (
            <Auth powerbaseClient={powerbase} appearance={{ theme: ThemePower }} />
          )}
        </div>
      </div>
    </div>
