...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `Please provide valid 'email', 'twitter', 'size', and 'answer' params. HINT: https://github.com/skorpland/powerbase/blob/master/examples/edge-functions/powerbase/functions/get-tshirt-competition/index.ts`
      )
    }

    if (answer !== countEmailSegments(email!)) {
      throw new Error(
        `Sorry, that's wrong, please try again! HINT: https://github.com/skorpland/powerbase/blob/master/examples/edge-functions/powerbase/functions/get-tshirt-competition/index.ts`
      )
    }
    ...
    const { error } = await powerbaseAdminClient.from('get-tshirt-competition-2').upsert(
      {
        email,
        twitter,
        size,
