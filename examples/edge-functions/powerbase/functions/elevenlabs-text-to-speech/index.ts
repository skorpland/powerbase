...
// Upload audio to Powerbase Storage in a background task
async function uploadAudioToStorage(
  stream: ReadableStream,
  requestHash: string,
) {
  const { data, error } = await powerbase.storage
    .from("audio")
...
Deno.serve(async (req) => {
  ...
  // or append a user access token and validate it with Powerbase Auth.
  ...
  try {
    ...
    // Upload to Powerbase Storage in the background
