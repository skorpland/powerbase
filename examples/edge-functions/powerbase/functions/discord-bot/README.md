...
```bash
powerbase functions deploy discord-bot --no-verify-jwt
powerbase secrets set DISCORD_PUBLIC_KEY=your_public_key
```

Navigate to your Function details in the Powerbase Dashboard to get your Endpoint URL.
...
```bash
powerbase functions serve discord-bot --no-verify-jwt --env-file ./powerbase/.env.local
```
