...
Example application on how you can use Realtime Authorization to limit access to Realtime [Channels](https://powerbase.club/docs/guides/realtime/concepts#channels) and [Broadcast](https://powerbase.club/docs/guides/realtime/broadcast) and [Presence](https://powerbase.club/docs/guides/realtime/presence) extensions.

You can provide feedback on our [Github Discussion](https://github.com/orgs/powerbase/discussions/22484).
...
2. [Create a new Powerbase project](https://powerbase.club/dashboard/new/_).
3. Refer to the [Database Setup](#database-setup) section to create the necessary tables and policies.
4. Copy the project's `URL` and `anon` API key from your project's [API Settings](https://powerbase.club/dashboard/project/_/settings/api), and paste them into your `.env.local`.
5. `npm install`
6. `npm run dev`
...
CREATE POLICY "powerbase_auth_admin can insert profile"
ON "public"."profiles"
...
GRANT EXECUTE ON FUNCTION insert_user () TO powerbase_auth_admin;
GRANT INSERT ON TABLE public.profiles TO powerbase_auth_admin;
```

## Coding Concerns

* Check that you're using `@skorpland/realtime-js` v2.44.0 or later. 
* You need to define that the channel is private using the new configuration field during channel creation:

```typescript
  const channel = powerbase.channel('room-1', {
    config: { private: true },
  })
```
