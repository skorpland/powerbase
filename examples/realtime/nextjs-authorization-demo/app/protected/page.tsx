...
import { User, createClientComponentClient } from '@skorpland/auth-helpers-nextjs'
import { RealtimeChannel } from '@skorpland/powerbase-js'
...
export default function Chat() {
  ...
  const powerbase = createClientComponentClient()

  const getChannels = async () => {
    const channels = await powerbase.from('rooms').select('topic')
    setRooms(channels.data?.map(({ topic }) => topic) || [])
  }

  const addUserToChannel = async (email: string) => {
    const user = await powerbase.from('profiles').select('id').eq('email', email)
    if (!user.data?.length) {
      addMessage(true, true, `User ${email} not found`)
    } else {
      const room = await powerbase.from('rooms').select('topic').eq('topic', selectedRoom)
  ...
  useEffect(() => {
    powerbase.auth
      ...
      .then(async () => {
        ...
        const token = (await powerbase.auth.getSession()).data.session?.access_token!
        powerbase.realtime.setAuth(token)
        let main = powerbase
          .channel('powerslack')
  ...
  useEffect(() => {
    ...
    if (selectedRoom) {
      ...
      let newChannel = powerbase.channel(selectedRoom, {
        config: {
          broadcast: { self: true },
