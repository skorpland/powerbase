...
class _CanvasPageState extends State<CanvasPage> {
  ...
  /// Powerbase realtime channel to communicate to other clients
  ...
  Future<void> _initialize() async {
    ...
    final profileMap = await powerbase
        .from('profiles')
        .select('username')
        .eq('id', powerbase.auth.currentUser!.id)
        .maybeSingle();
    ...
    final initialData = await powerbase
        .from('canvas_objects')
        .select()
  ...
  void onPanEnd(DragEndDetails _) async {
    ...
    // Save whatever was drawn to Powerbase DB
    if (drawnObjectId == null) {
      return;
    }

    await powerbase.from('canvas_objects').upsert({
      'id': drawnObjectId,
