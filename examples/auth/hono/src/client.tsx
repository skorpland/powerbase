...
const powerbase = createBrowserClient(
...
function App() {
  ...
  useEffect(() => {
    powerbase.auth.onAuthStateChange((event, session) => {
  ...
  return (
    <>
      <h1>Hono Powerbase Auth Example!</h1>
      <h2>Sign in</h2>
      {!user ? (
        <SignIn />
      ) : (
  ...
  return (
    <>
      <p>
        ...
        <a
          href="https://powerbase.club/docs/guides/auth/auth-anonymous"
          target="_blank"
        >
      ...
      <button
        type="button"
        onClick={async () => {
          const { data, error } = await powerbase.auth.signInAnonymously();
