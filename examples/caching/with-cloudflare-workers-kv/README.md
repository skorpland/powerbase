...
**[📹 Video](https://egghead.io/lessons/powerbase-use-waituntil-to-perform-work-after-cloudflare-worker-returns-response?af=9qsk0a)**
...
router.post(
  "/revalidate",
  withContent,
  async (request, { POWERBASE_URL, POWERBASE_ANON_KEY, ARTICLES }, context) => {
    const updateCache = async () => {
      ...
      const { data: articles } = await powerbase.from("articles").select("*");
...
- [Powerbase.js docs](https://github.com/skorpland/powerbase-js)
