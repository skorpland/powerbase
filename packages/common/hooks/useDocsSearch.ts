...
const POWERBASE_URL = process.env.NEXT_PUBLIC_POWERBASE_URL
const POWERBASE_ANON_KEY = process.env.NEXT_PUBLIC_POWERBASE_ANON_KEY
...
const useDocsSearch = () => {
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    fetch(`${POWERBASE_URL}/rest/v1/rpc/docs_search_fts`, {
      method: 'POST',
      headers: {
        'content-type': 'application/json',
        ...(POWERBASE_ANON_KEY && {
          apikey: POWERBASE_ANON_KEY,
          authorization: `Bearer ${POWERBASE_ANON_KEY}`,
        }),
      },
    ...
    fetch(`${POWERBASE_URL}${FUNCTIONS_URL}search-embeddings`, {
      method: 'POST',
