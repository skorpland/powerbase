...
export default function SqlToRest({
  ...
  useLayoutEffect(() => {
    if (monaco) {
      ...
      monaco.editor.defineTheme('powerbase-light', lightMode)
      monaco.editor.defineTheme('powerbase-dark', darkMode)
    }
  }, [monaco])
  ...
  const [jsQuery, setJsQuery] = useState<PowerbaseJsQuery>()
  ...
  const [powerbaseJsRenderError, setPowerbaseJsRenderError] = useState<RenderError>()
  ...
  const process = useCallback(async (sql: string) => {
    ...
    try {
      ...
      if (error instanceof ParsingError) {
        ...
        } else if (error.renderer === 'powerbase-js') {
          setPowerbaseJsRenderError(error)
        } else {
          console.error(error)
        }
      } else {
    ...
    <div className="grid grid-cols-1 2xl:grid-cols-2 gap-4 mt-4">
      <div className="flex flex-col gap-4">
        ...
        >
          <Editor
            language="pgsql"
            theme={isDark ? 'powerbase-dark' : 'powerbase-light'}
            value={sql}
            options={{
              tabSize: 2,
              minimap: {
        ...
        {unimplementedError && (
          <Alert className="text-orange-1000">
            ...
            <div className="mt-2 text-white flex gap-1 leading-6">
              ...
              <a
                href="https://github.com/skorpland/sql-to-rest/issues/new/choose"
                target="_blank"
      ...
      >
        ...
        <Tabs activeId={currentLanguage} onChange={(id: string) => setCurrentLanguage(id)}>
          ...
          <Tabs.Panel id="js" label="JavaScript" className="flex flex-col gap-4">
            {powerbaseJsRenderError && (
              <Alert className="text-red-900">{powerbaseJsRenderError.message}</Alert>
            )}
            <CodeBlock
              language="js"
              hideLineNumbers
              className={cn(
                'self-stretch overflow-y-hidden',
                powerbaseJsRenderError ? 'opacity-25 pointer-events-none' : ''
              )}
              renderer={codeBlockRenderer}
            >
              {jsCommand}
        ...
        <div
          className={cn(
            ...
            ((currentLanguage === 'http' || currentLanguage === 'curl') && httpRenderError) ||
              (currentLanguage === 'js' && powerbaseJsRenderError)
              ? 'opacity-25 pointer-events-none'
              : ''
          )}
        >
