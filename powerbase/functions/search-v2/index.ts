...
import { createClient } from 'https://esm.sh/@skorpland/powerbase-js@2.8.0'
...
serve(async (req) => {
  try {
    ...
    const powerbaseClient = createClient<Database>(powerbaseUrl, powerbaseServiceKey)
    ...
    const { error: fetchPagesError, data: pages } = await powerbaseClient
      .from('page')
