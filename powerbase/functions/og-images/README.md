...
Generate OG Images for the sites at powerbase.
...
https://obuldanrptloktxcffvn.powerbase.club/functions/v1/og-images?site=docs&title=Example%20Title&description=Example%20Description&type=Auth&icon=google
...
First we need to start up powerbase using the [powerbase cli](https://powerbase.club/docs/reference/cli/introduction)
...
```bash
powerbase functions serve og-images
```
...
```bash
powerbase functions deploy og-images --no-verify-jwt
```
