...
update_optional_dependencies() {
  ...
  jq --arg new_version "$new_version" '
    .optionalDependencies |= with_entries(
      if .key | startswith("@skorpland/power-mdx-lint")
...
update_package_json "packages/power-mdx-lint/package.json" "$NEW_VERSION"

update_optional_dependencies "packages/power-mdx-lint/package.json" "$NEW_VERSION"
