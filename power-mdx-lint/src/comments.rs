...
impl<'comment> ConfigurationComment<'comment> {
    fn parse(value: &'comment str) -> Option<Self> {
        ...
        // power-mdx-lint configure-next-line Rule001HeadingCase +Powerbase +pgjwt
        let regex = CONFIG_COMMENT_REGEX.get_or_init(||
            Regex::new(r"^power-mdx-lint-(enable|disable|disable-next-line|configure|configure-next-line)(?:\s+(\S+)(?:\s+(.+))?)?$").expect("Hardcoded regex should not fail")
        );
...
"#;
    ...
    fn rule_toggle_parse_enable_all() {
        let value = "/* power-mdx-lint-enable */";
        assert!(matches!(
    ...
    fn rule_toggle_parse_enable_specific_rule() {
        let value = "/* power-mdx-lint-enable specific-rule */";
        assert!(matches!(
            ConfigurationComment::parse(value),
    ...
    fn rule_toggle_parse_disable_all() {
        let value = "/* power-mdx-lint-disable */";
        assert!(matches!(
            ConfigurationComment::parse(value),
    ...
    fn rule_toggle_parse_disable_specific_rule() {
        let value = "/* power-mdx-lint-disable specific-rule */";
        assert!(matches!(
            ConfigurationComment::parse(value),
    ...
    fn rule_toggle_parse_disable_next_line_all() {
        let value = "/* power-mdx-lint-disable-next-line */";
        assert!(matches!(
            ConfigurationComment::parse(value),
            Some(ConfigurationComment::EnableDisable(RuleToggle::DisableAll { next_line_only }))
            if next_line_only
    ...
    fn rule_toggle_parse_disable_next_line_specific_rule() {
        let value = "/* power-mdx-lint-disable-next-line specific-rule */";
        assert!(matches!(
            ConfigurationComment::parse(value),
            Some(ConfigurationComment::EnableDisable(RuleToggle::DisableRule { rule, next_line_only }))
            if rule == "specific-rule" && next_line_only
    ...
    fn rule_toggle_parse_invalid_command() {
        let value = "/* power-mdx-lint-invalid */";
        assert!(ConfigurationComment::parse(value).is_none());
    }

    #[test]
    fn rule_toggle_parse_ignores_whitespace() {
        let value = "     /*     power-mdx-lint-enable  rule-name  */";
        assert!(matches!(
            ConfigurationComment::parse(value),
            Some(ConfigurationComment::EnableDisable(RuleToggle::EnableRule { rule }))
            if rule == "rule-name"
    ...
    fn test_collect_lint_disables_basic() {
        let input = r#"{/* power-mdx-lint-disable foo */}
Some content
{/* power-mdx-lint-enable foo */}"#;
    ...
    fn test_collect_lint_disables_multiple_rules() {
        let input = r#"{/* power-mdx-lint-disable foo */}
Content
{/* power-mdx-lint-disable bar */}
More content
{/* power-mdx-lint-enable foo */}
{/* power-mdx-lint-enable bar */}"#;
    ...
    fn test_collect_lint_disables_next_line() {
        let input = r#"{/* power-mdx-lint-disable-next-line foo */}
...
This line is not ignored"#;
    ...
    fn test_collect_lint_disables_disable_all() {
        let input = r#"{/* power-mdx-lint-disable */}
Everything here is ignored
Still ignored
{/* power-mdx-lint-enable */}"#;
...
Never reenabled"#;
    ...
    fn test_collect_lint_disables_invalid_enable() {
        let input = r#"{/* power-mdx-lint-enable foo */}
...
{/* some other comment */}
{/* power-mdx-lint-disable-next-line bar */}
...
{/* power-mdx-lint-disable-next-line foo */}
