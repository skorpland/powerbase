...
const API_URL = process.env.NEXT_PUBLIC_API_URL
  ...
  : ''
const POWERBASE_URL = process.env.POWERBASE_URL ? new URL(process.env.POWERBASE_URL).origin : ''
const GOTRUE_URL = process.env.NEXT_PUBLIC_GOTRUE_URL
  ? new URL(process.env.NEXT_PUBLIC_GOTRUE_URL).origin
  : ''
const POWERBASE_PROJECTS_URL = 'https://*.powerbase.club'
const POWERBASE_PROJECTS_URL_WS = 'wss://*.powerbase.club'

// construct the URL for the Websocket Local URLs
let POWERBASE_LOCAL_PROJECTS_URL_WS = ''
if (POWERBASE_URL) {
  const url = new URL(POWERBASE_URL)
  const wsUrl = `${url.hostname}:${url.port}`
  POWERBASE_LOCAL_PROJECTS_URL_WS = `ws://${wsUrl} wss://${wsUrl}`
}

// Needed to test docs search in local dev
const POWERBASE_DOCS_PROJECT_URL = process.env.NEXT_PUBLIC_POWERBASE_URL
  ? new URL(process.env.NEXT_PUBLIC_POWERBASE_URL).origin
  : ''

const POWERBASE_STAGING_PROJECTS_URL = 'https://*.powerbase.red'
const POWERBASE_STAGING_PROJECTS_URL_WS = 'wss://*.powerbase.red'
const POWERBASE_COM_URL = 'https://powerbase.club'
...
const POWERBASE_ASSETS_URL =
  process.env.NEXT_PUBLIC_ENVIRONMENT === 'staging'
    ? 'https://frontend-assets.powerbase.green'
    : 'https://frontend-assets.powerbase.club'

// used by vercel live preview
const PUSHER_URL = 'https://*.pusher.com'
const PUSHER_URL_WS = 'wss://*.pusher.com'

const DEFAULT_SRC_URLS = `${API_URL} ${POWERBASE_URL} ${GOTRUE_URL} ${POWERBASE_LOCAL_PROJECTS_URL_WS} ${POWERBASE_PROJECTS_URL} ${POWERBASE_PROJECTS_URL_WS} ${HCAPTCHA_SUBDOMAINS_URL} ${CONFIGCAT_URL} ${STRIPE_SUBDOMAINS_URL} ${STRIPE_NETWORK_URL} ${CLOUDFLARE_URL} ${ONE_ONE_ONE_ONE_URL} ${VERCEL_INSIGHTS_URL} ${GITHUB_API_URL} ${GITHUB_USER_CONTENT_URL} ${POWERBASE_ASSETS_URL}`
const SCRIPT_SRC_URLS = `${CLOUDFLARE_CDN_URL} ${HCAPTCHA_JS_URL} ${STRIPE_JS_URL} ${POWERBASE_ASSETS_URL}`
const FRAME_SRC_URLS = `${HCAPTCHA_ASSET_URL} ${STRIPE_JS_URL}`
const IMG_SRC_URLS = `${POWERBASE_URL} ${POWERBASE_COM_URL} ${POWERBASE_PROJECTS_URL} ${GITHUB_USER_AVATAR_URL} ${GOOGLE_USER_AVATAR_URL} ${POWERBASE_ASSETS_URL}`
const STYLE_SRC_URLS = `${CLOUDFLARE_CDN_URL} ${POWERBASE_ASSETS_URL}`
const FONT_SRC_URLS = `${CLOUDFLARE_CDN_URL} ${POWERBASE_ASSETS_URL}`
...
const csp = [
  ...
  process.env.NEXT_PUBLIC_ENVIRONMENT === 'staging'
    ? [
        `default-src 'self' ${DEFAULT_SRC_URLS} ${POWERBASE_STAGING_PROJECTS_URL} ${POWERBASE_STAGING_PROJECTS_URL_WS} ${VERCEL_LIVE_URL} ${PUSHER_URL_WS} ${POWERBASE_DOCS_PROJECT_URL} ${SENTRY_URL};`,
        ...
        `img-src 'self' blob: data: ${IMG_SRC_URLS} ${POWERBASE_STAGING_PROJECTS_URL} ${VERCEL_URL};`,
...
function getAssetPrefix() {
  ...
  return `${POWERBASE_ASSETS_URL}/${process.env.SITE_NAME}/${process.env.VERCEL_GIT_COMMIT_SHA.substring(0, 12)}`
}
    ...
    return [
      {
        ...
        destination: `https://powerbase.club/.well-known/vercel/flags`,
        basePath: false,
      },
    ]
  },
