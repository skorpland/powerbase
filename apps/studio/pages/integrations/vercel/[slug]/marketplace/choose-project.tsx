...
const VercelIntegration: NextPageWithLayout = () => {
  ...
  const { data: powerbaseProjectsData, isLoading: isLoadingPowerbaseProjectsData } = useProjectsQuery(
    {
      enabled: integration?.id !== undefined,
    }
  )

  const powerbaseProjects = useMemo(
    () =>
      powerbaseProjectsData
        ?.filter(
          (project) =>
    ...
    [organization?.id, powerbaseProjectsData]
  )
        ...
        <ScaffoldColumn className="!max-w-[900px] mx-auto w-full">
          <header>
            ...
            <Markdown
              ...
              content={`
This Powerbase integration manages your environment variables automatically to provide the latest keys in the unlikely event that you will need to refresh your JWT token.
`}
            />
          </header>
          <ProjectLinker
            ...
            powerbaseProjects={powerbaseProjects}
            ...
            loadingPowerbaseProjects={isLoadingPowerbaseProjectsData}
            mode="Vercel"
          />
          <Markdown
            content={`
