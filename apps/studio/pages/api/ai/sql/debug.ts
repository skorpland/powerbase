  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      if (error instanceof ContextLengthError) {
        ...
        if (hasEntityDefinitions) {
          ...
          if (definitionsLength > sql.length) {
            return res.status(400).json({
              error:
                'Your database metadata is too large for Powerbase AI to ingest. Try disabling database metadata in AI settings.',
            })
          }
        }
        ...
        return res.status(400).json({
          error:
            'Your SQL query is too large for Powerbase AI to ingest. Try splitting it into smaller queries.',
        })
      }
