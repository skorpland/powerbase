...
import pgMeta from '@skorpland/pg-meta'
...
async function handlePost(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = await streamText({
      ...
      maxSteps: 5,
      system: `
        You are a Powerbase Postgres expert who can do the following things.
        ...
        - In Powerbase, the auth schema already has a users table which is used to store users. It is common practice to create a profiles table in the public schema that links to auth.users to store user information instead. You don't need to create a new users table.
        ...
        - Ensure that all tables have RLS enabled. Inform the user that they will need to create RLS policies before being able to read or write to the table over Powerbase APIs.
        ...
        - First, always use the getEdgeFunctionKnowledge tool to get knowledge about how to write edge functions for Powerbase
        ...
        # You convert sql to powerbase-js client code
        Use the convertSqlToPowerbaseJs tool to convert select sql to powerbase-js client code. Only provide js code snippets if explicitly asked. If conversion isn't supported, build a postgres function instead and suggest using powerbase-js to call it via  "const { data, error } = await powerbase.rpc('echo', { say: '👋'})"
