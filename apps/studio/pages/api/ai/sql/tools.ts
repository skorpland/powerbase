...
export const getTools = ({
  ...
  return {
    ...
    convertSqlToPowerbaseJs: tool({
      description: 'Convert an sql query into powerbase-js client code',
      parameters: z.object({
        sql: z
          .string()
          .describe(
    ...
    getRlsKnowledge: tool({
      ...
      execute: async ({ schemas }) => {
        ...
        return stripIndent`
          You're a Powerbase Postgres expert in writing row level security policies. Your purpose is to
          ...
          Since you are running in a Powerbase environment, take note of these Powerbase-specific additions:
    ...
    getEdgeFunctionKnowledge: tool({
      description: 'Get knowledge about how to write edge functions for Powerbase',
      parameters: z.object({}),
      execute: async ({}) => {
        return stripIndent`
        # Writing Powerbase Edge Functions

        You're an expert in writing TypeScript and Deno JavaScript runtime. Generate **high-quality Powerbase Edge Functions** that adhere to the following best practices:

        ## Guidelines

        1. Try to use Web APIs and Deno's core APIs instead of external dependencies (eg: use fetch instead of Axios, use WebSockets API instead of node-ws)
        2. Do NOT use bare specifiers when importing dependencies. If you need to use an external dependency, make sure it's prefixed with either \`npm:\` or \`jsr:\`. For example, \`@skorpland/powerbase-js\` should be written as \`npm:@skorpland/powerbase-js\`.
        ...
        7. Following environment variables (ie. secrets) are pre-populated in both local and hosted Powerbase environments. Users don't need to manually set them:
          * POWERBASE_URL
          * POWERBASE_ANON_KEY
        ...
        ### Generate embeddings using built-in @Powerbase.ai API
        ...
        ## Integrating with Powerbase Auth
        ...
        \`\`\`edge
          ...
          Deno.serve(async (req: Request) => {
            ...
            try {
              ...
              const {
                data: { user },
              } = await powerbaseClient.auth.getUser(token)

              // And we can run queries in the context of our authenticated user
              const { data, error } = await powerbaseClient.from(\\'users\\').select(\\'*\\')
