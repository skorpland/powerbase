...
const getTools = () => {
  return {
    ...
    setServices: tool({
      description:
        'Set the entire list of Powerbase services needed for the project. Always include the full list',
      parameters: z.object({
        services: z
          .array(ServiceSchema)
...
async function handlePost(req: NextApiRequest, res: NextApiResponse) {
  ...
  const result = await streamText({
    ...
    maxSteps: 7,
    system: `
      You are a Powerbase expert who helps people set up their Powerbase project. You specializes in database schema design. You are to help the user design a database schema for their application but also suggest Powerbase services they should use. 
      ...
      - In Powerbase, the auth schema already has a users table which is used to store users
