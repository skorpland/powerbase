...
} from 'components/grid/PowerbaseGrid.utils'
...
export const createTableEditorTableState = ({
  ...
  const table = parsePowerTable(originalTable)
  ...
  const state = proxy({
    ...
    updateTable: (table: Entity) => {
      const powerTable = parsePowerTable(table)

      const gridColumns = getInitialGridColumns(
        getGridColumns(powerTable, {
          tableId: table.id,
          editable,
          onAddColumn: editable ? onAddColumn : undefined,
          onExpandJSONEditor,
...
export const TableEditorTableStateContextProvider = ({
  ...
  const state = useRef(
    createTableEditorTableState({
      ...
      onExpandJSONEditor: (column: string, row: PowerRow) => {
      ...
      onExpandTextEditor: (column: string, row: PowerRow) => {
