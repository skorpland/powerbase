...
type UpdateVariables = {
  ...
  workdir: string
  powerbaseChangesOnly: boolean
  branchLimit: number
}

export async function updateConnection(
  { connectionId, workdir, powerbaseChangesOnly, branchLimit }: UpdateVariables,
  signal?: AbortSignal
) {
  const { data, error } = await patch('/platform/integrations/github/connections/{connection_id}', {
    ...
    signal,
    body: { workdir, powerbase_changes_only: powerbaseChangesOnly, branch_limit: branchLimit },
  })
