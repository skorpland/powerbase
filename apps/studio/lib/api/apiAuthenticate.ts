...
import { readOnly } from './powerbaseClient'
import type { PowerResponse, User } from 'types'
...
export async function apiAuthenticate(
  req: NextApiRequest,
  res: NextApiResponse
): Promise<PowerResponse<User>> {
  if (!req) {
    return { error: new Error('Request is not available') } as unknown as PowerResponse<User>
  }

  if (!res) {
    return { error: new Error('Response is not available') } as unknown as PowerResponse<User>
  }
  ...
  try {
    ...
    if (!user) {
      return { error: new Error('The user does not exist') } as unknown as PowerResponse<User>
    }
    ...
    return { error: { message: error.message ?? 'unknown' } } as unknown as PowerResponse<User>
  }
}
...
async function fetchUser(req: NextApiRequest, res: NextApiResponse): Promise<any> {
  ...
  if (user_id_powerbase) {
    return {
      id: user_id_powerbase,
      primary_email: email,
    }
  }
