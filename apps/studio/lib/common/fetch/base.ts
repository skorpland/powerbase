...
export async function handleResponse<T>(
  ...
  try {
    ...
    return handleError(response, requestId) as PowerResponse<T>
  }
}
...
export async function handleResponseError<T = unknown>(
  ...
  if (resJson.error && typeof resJson.error === 'string') {
    ...
    return { error: { code: response.status, ...resJson.error } } as unknown as PowerResponse<T>
  } else {
...
export function isResponseOk<T>(response: PowerResponse<T> | undefined): response is T {
  return (
    response !== undefined &&
    response !== null &&
