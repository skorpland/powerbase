...
import { ResponseError, type PowerResponse } from 'types'
import { isResponseOk } from './common/fetch'

async function fetchGitHub<T = any>(url: string, responseJson = true): Promise<PowerResponse<T>> {
 ...
 * @param externalId An external GitHub URL for example: https://github.com/vercel/next.js/tree/canary/examples/with-powerbase
 */
export async function getInitialMigrationSQLFromGitHubRepo(
  ...
  const [powerbaseFilesResponse, migrationFilesResponse] = await Promise.all([
    fetchGitHub<File[]>(powerbaseFolderUrl),
    fetchGitHub<File[]>(migrationsFolderUrl),
  ])

  if (!isResponseOk(powerbaseFilesResponse)) {
    console.warn(`Failed to fetch powerbase files from GitHub: ${powerbaseFilesResponse.error}`)
    return null
  }
  ...
  const seedFileUrl = powerbaseFilesResponse.find((file) => file.name === 'seed.sql')?.download_url
  ...
  const migrationsTableSql = /* SQL */ `
    create schema if not exists powerbase_migrations;
    create table if not exists powerbase_migrations.schema_migrations (
    ...
    ${sortedFiles.map((file, i) => {
      ...
      return /* SQL */ `
        insert into powerbase_migrations.schema_migrations (version, statements, name)
