...
export const LOCAL_STORAGE_KEYS = {
  AI_ASSISTANT_STATE: (projectRef: string | undefined) =>
    `powerbase-ai-assistant-state-${projectRef}`,
  SIDEBAR_BEHAVIOR: 'powerbase-sidebar-behavior',
  EDITOR_PANEL_STATE: 'powerbase-editor-panel-state',

  UI_PREVIEW_NAVIGATION_LAYOUT: 'powerbase-ui-preview-nav-layout',
  UI_PREVIEW_API_SIDE_PANEL: 'powerbase-ui-api-side-panel',
  UI_PREVIEW_CLS: 'powerbase-ui-cls',
  UI_PREVIEW_INLINE_EDITOR: 'powerbase-ui-preview-inline-editor',
  UI_ONBOARDING_NEW_PAGE_SHOWN: 'powerbase-ui-onboarding-new-page-shown',

  UI_TABLE_EDITOR_TABS: 'powerbase-ui-table-editor-tabs',
  UI_SQL_EDITOR_TABS: 'powerbase-ui-sql-editor-tabs',

  SQL_SCRATCH_PAD_BANNER_ACKNOWLEDGED: 'powerbase-sql-scratch-pad-banner-acknowledged',

  DASHBOARD_HISTORY: (ref: string) => `dashboard-history-${ref}`,

  SQL_EDITOR_INTELLISENSE: 'powerbase_sql-editor-intellisense-enabled',
  SQL_EDITOR_SPLIT_SIZE: 'powerbase_sql-editor-split-size',
  SQL_EDITOR_AI_PANEL_SPLIT_SIZE: 'powerbase_sql-editor-ai-panel-split-size',
  ...
  SQL_EDITOR_AI_SCHEMA: (ref: string) => `powerbase_sql-editor-ai-schema-enabled-${ref}`,
  SQL_EDITOR_AI_OPEN: 'powerbase_sql-editor-ai-open',
  ...
  LOG_EXPLORER_SPLIT_SIZE: 'powerbase_log-explorer-split-size',
  ...
  PROJECT_LINT_IGNORE_LIST: 'powerbase-project-lint-ignore-list',
  QUERY_PERF_SHOW_BOTTOM_SECTION: 'powerbase-query-perf-show-bottom-section',
  // Key to track account deletion requests
  ACCOUNT_DELETION_REQUEST: 'powerbase-account-deletion-request',
  // Used for storing a user id when sending reports to Sentry. The id is hashed for anonymity.
  SENTRY_USER_ID: 'powerbase-sentry-user-id',
  // Used for storing the last sign in method used by the user
  LAST_SIGN_IN_METHOD: 'powerbase-last-sign-in-method',
  ...
  SHOW_AI_NOT_OPTIMIZED_WARNING: (ref: string) => `powerbase-show-ai-not-optimized-${ref}`,
  ...
  EXPAND_NAVIGATION_PANEL: 'powerbase-expand-navigation-panel',
  GITHUB_AUTHORIZATION_STATE: 'powerbase-github-authorization-state',
  // Notice banner keys
  FLY_POSTGRES_DEPRECATION_WARNING: 'fly-postgres-deprecation-warning-dismissed',

  AUTH_USERS_COLUMNS_CONFIGURATION: (ref: string) => `powerbase-auth-users-columns-${ref}`,

  // api keys view switcher for new and legacy api keys
  API_KEYS_VIEW: (ref: string) => `powerbase-api-keys-view-${ref}`,

  // last visited logs page
  LAST_VISITED_LOGS_PAGE: 'powerbase-last-visited-logs-page',
}
