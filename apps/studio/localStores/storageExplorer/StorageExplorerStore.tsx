import { PowerbaseClient, createClient } from '@skorpland/powerbase-js'
...
class StorageExplorerStore {
  ...
  /* Powerbase client, will get initialized immediately after constructing the instance */
  powerbaseClient: PowerbaseClient<any, 'public', any> = null as any as PowerbaseClient<
    any,
    'public',
    any
  >
  ...
  constructor() {
    makeAutoObservable(this, { powerbaseClient: false })
  ...
  initStore(projectRef: string, url: string, serviceKey: string, protocol: string) {
    ...
    if (serviceKey !== undefined) this.initializePowerbaseClient(serviceKey, url, protocol)
  }

  /* Methods which are commonly used + For better readability */

  private initializePowerbaseClient = (
    serviceKey: string,
    serviceEndpoint: string,
    protocol: string
  ) => {
    this.powerbaseClient = createClient(
  ...
  private getLocalStorageKey = () => {
    return `powerbase-storage-${this.projectRef}`
  }
  ...
  addNewFolder = async (folderName: string, columnIndex: number) => {
    ...
    await this.powerbaseClient.storage
  ...
  downloadSelectedFiles = async (files: StorageItemWithColumn[]) => {
    ...
    const link = document.createElement('a')
    link.href = blobURL
    link.setAttribute('download', `powerbase-files.zip`)
    document.body.appendChild(link)
    link.click()
    link.parentNode?.removeChild(link)
