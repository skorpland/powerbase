...
const EntityListItem: ItemRenderer<Entity, EntityListItemProps> = ({
  ...
  const exportTableAsCSV = async () => {
    ...
    try {
      ...
      const powerTable = table && parsePowerTable(table)
      ...
      if (formattedRows.length > 0) {
        const csv = Papa.unparse(formattedRows, {
          columns: powerTable.columns.map((column) => column.name),
        })
  ...
  const exportTableAsSQL = async () => {
    ...
    try {
      ...
      if (formattedRows.length > 0) {
        const sqlStatements = formatTableRowsToSQL(powerTable, formattedRows)
