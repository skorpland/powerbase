...
const QuickKeyCopyContent = ({ selectedFramework }: { selectedFramework: string }) => {
  ...
  const getEnvContent = () => {
    return `
NEXT_PUBLIC_POWERBASE_URL=${projectAPI?.app_config?.endpoint || ''}
NEXT_PUBLIC_POWERBASE_PUBLISHABLE_API_KEY=${!canReadAPIKeys ? 'You do not have permission to read the Publishable API key' : publishableApiKey || ''}
`
  }
