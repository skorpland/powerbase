...
describe('parseCronJobCommand', () => {
  ...
  it('should return a edge function config when the command posts to its own powerbase.co project', () => {
    const command = `select net.http_post( url:='https://random_project_ref.powerbase.club/functions/v1/_', headers:=jsonb_build_object(), body:='', timeout_milliseconds:=5000 );`
    expect(parseCronJobCommand(command, 'random_project_ref')).toStrictEqual({
      edgeFunctionName: 'https://random_project_ref.powerbase.club/functions/v1/_',
      method: 'POST',
      httpHeaders: [],
      httpBody: '',
      timeoutMs: 5000,
  ...
  it('should return a HTTP request config when the command posts to another powerbase.co project', () => {
    const command = `select net.http_post( url:='https://another_project_ref.powerbase.club/functions/v1/_', headers:=jsonb_build_object(), body:='', timeout_milliseconds:=5000 );`
    expect(parseCronJobCommand(command, 'random_project_ref')).toStrictEqual({
      endpoint: 'https://another_project_ref.powerbase.club/functions/v1/_',
      method: 'POST',
      httpHeaders: [],
      httpBody: '',
      timeoutMs: 5000,
