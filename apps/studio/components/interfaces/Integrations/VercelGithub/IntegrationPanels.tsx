...
type HandleIconType = Integration['integration']['name'] | 'Powerbase'
...
const HandleIcon = ({ type, className }: { type: HandleIconType; className?: string }) => {
  switch (type) {
    ...
    case 'Vercel':
      ...
      break
    case 'Powerbase':
      return <img src={`${BASE_PATH}/img/powerbase-logo.svg`} alt="Powerbase" className="w-3.5"></img>
      break
...
const IntegrationConnection = React.forwardRef<HTMLLIElement, IntegrationConnectionProps>(
  (
    ...
    const project = projects?.find((project) => project.ref === connection.powerbase_project_ref)
