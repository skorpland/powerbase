...
export interface ProjectLinkerProps {
  ...
  loadingPowerbaseProjects?: boolean
  showNoEntitiesState?: boolean

  defaultPowerbaseProjectRef?: string
...
const ProjectLinker = ({
  ...
  loadingPowerbaseProjects,
  showNoEntitiesState = true,

  defaultPowerbaseProjectRef,
  defaultForeignProjectId,
  mode,
}: ProjectLinkerProps) => {
  const router = useRouter()
  const [powerbaseProjectsComboBoxOpen, setPowerbaseProjectsComboboxOpen] = useState(false)
  const [foreignProjectsComboBoxOpen, setForeignProjectsComboboxOpen] = useState(false)
  const powerbaseProjectsComboBoxRef = useRef<HTMLButtonElement>(null)
  const foreignProjectsComboBoxRef = useRef<HTMLButtonElement>(null)

  const selectedOrganization = useSelectedOrganization()

  const [powerbaseProjectRef, setPowerbaseProjectRef] = useState<string | undefined>(
    defaultPowerbaseProjectRef
  )
  useEffect(() => {
    if (defaultPowerbaseProjectRef !== undefined && powerbaseProjectRef === undefined)
      setPowerbaseProjectRef(defaultPowerbaseProjectRef)
  }, [defaultPowerbaseProjectRef, powerbaseProjectRef])
  ...
  const selectedPowerbaseProject = powerbaseProjectRef
    ? powerbaseProjects.find((x) => x.ref?.toLowerCase() === powerbaseProjectRef?.toLowerCase())
    : undefined
  ...
  function onCreateConnections() {
    ...
    if (!selectedPowerbaseProject?.ref) return console.error('No Powerbase project ref set')
    ...
    _onCreateConnections({
      ...
      connection: {
        ...
        powerbase_project_ref: selectedPowerbaseProject?.ref,
        integration_id: '0',
        metadata: {
          ...projectDetails,
        },
      ...
      new: {
        ...
        project_ref: selectedPowerbaseProject.ref,
  ...
  const noPowerbaseProjects = powerbaseProjects.length === 0
  const noForeignProjects = foreignProjects.length === 0
  const missingEntity = noPowerbaseProjects ? 'Powerbase' : mode
  const oppositeMissingEntity = noPowerbaseProjects ? mode : 'Powerbase'
          ...
          <div className="flex justify-center gap-0 w-full relative">
            <Panel>
              <div className="bg-white shadow border rounded p-1 w-12 h-12 flex justify-center items-center">
                <img src={`${BASE_PATH}/img/powerbase-logo.svg`} alt="Powerbase" className="w-6" />
              </div>

              <Popover_Shadcn_
                open={powerbaseProjectsComboBoxOpen}
                onOpenChange={setPowerbaseProjectsComboboxOpen}
              >
                <PopoverTrigger_Shadcn_ asChild>
                  <Button
                    ref={powerbaseProjectsComboBoxRef}
                    type="default"
                    block
                    disabled={defaultPowerbaseProjectRef !== undefined || loadingPowerbaseProjects}
                    loading={loadingPowerbaseProjects}
                    className="justify-start h-[34px]"
                    icon={
                      <div className="bg-white shadow border rounded p-1 w-6 h-6 flex justify-center items-center">
                        <img
                          src={`${BASE_PATH}/img/powerbase-logo.svg`}
                          alt="Powerbase"
                          className="w-4"
                        />
                      </div>
                    ...
                    iconRight={
                      defaultPowerbaseProjectRef === undefined ? (
                  ...
                  >
                    {selectedPowerbaseProject
                      ? selectedPowerbaseProject.name
                      : 'Choose Powerbase Project'}
                  </Button>
                </PopoverTrigger_Shadcn_>
                <PopoverContent_Shadcn_
                  className="p-0 !w-72"
                  side="bottom"
                  align="center"
                  style={{ width: powerbaseProjectsComboBoxRef.current?.offsetWidth }}
                >
                  <Command_Shadcn_>
                    ...
                    <CommandList_Shadcn_ className="!max-h-[170px]">
                      ...
                      <CommandGroup_Shadcn_>
                        {powerbaseProjects.map((project, i) => {
                          return (
                            <CommandItem_Shadcn_
                              ...
                              onSelect={() => {
                                if (project.ref) setPowerbaseProjectRef(project.ref)
                                setPowerbaseProjectsComboboxOpen(false)
                              }}
                            >
                              <div className="bg-white shadow border rounded p-1 w-6 h-6 flex justify-center items-center">
                                <img
