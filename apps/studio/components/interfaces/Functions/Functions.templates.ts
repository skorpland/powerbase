export const EDGE_FUNCTION_TEMPLATES = [
  {
    ...
    content: `// Setup type definitions for built-in Powerbase Runtime APIs
...
Deno.serve(async (req: Request) => {
  ...
  },
  {
    ...
    name: 'Powerbase Database Access',
    description: 'Example using Powerbase client to query your database',
    content: `// Setup type definitions for built-in Powerbase Runtime APIs
import "jsr:@skorpland/functions-js/edge-runtime.d.ts";
...
Deno.serve(async (req) => {
  try {
    ...
    const { data, error } = await powerbase.from('table_name').select('*')
  ...
  },
  {
    ...
    name: 'Powerbase Storage Upload',
    description: 'Upload files to Powerbase Storage',
    content: `// Setup type definitions for built-in Powerbase Runtime APIs
import "jsr:@skorpland/functions-js/edge-runtime.d.ts";
import { createClient } from 'jsr:@skorpland/powerbase-js@2'
...
// https://powerbase.club/docs/guides/functions/routing?queryGroups=framework&framework=expressjs
...
Deno.serve((req) => {
  ...
  socket.onopen = () => {
    ...
    socket.send('Welcome to Powerbase Edge Functions!')
  }
  socket.onmessage = (e) => {
