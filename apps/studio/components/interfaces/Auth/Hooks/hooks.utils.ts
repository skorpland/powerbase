...
export const getRevokePermissionStatements = (schema: string, functionName: string): string[] => {
  return [
    `-- Revoke access to function from powerbase_auth_admin\nrevoke execute on function ${schema}.${functionName} from powerbase_auth_admin;`,
    `-- Revoke access to schema from powerbase_auth_admin\nrevoke usage on schema ${schema} from powerbase_auth_admin;`,
