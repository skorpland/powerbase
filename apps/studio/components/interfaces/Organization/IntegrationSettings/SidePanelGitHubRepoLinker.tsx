...
const SidePanelGitHubRepoLinker = ({ projectRef }: SidePanelGitHubRepoLinkerProps) => {
  ...
  /**
   * Powerbase projects available
   */
  const { data: powerbaseProjectsData, isLoading: isLoadingPowerbaseProjects } = useProjectsQuery({
    enabled: visible,
  })
  ...
  const powerbaseProjects = useMemo(
    ...
    [selectedOrganization?.id, powerbaseProjectsData]
  )
          ...
          {gitHubAuthorization === null ? (
            <div className="flex flex-col items-center justify-center mt-8 relative border rounded-lg p-12 bg shadow px-20s">
              <p className="text-sm text-center">
                Connect your Powerbase projects with your GitHub repositories
              </p>
            ...
            <ProjectLinker
              defaultPowerbaseProjectRef={projectRef}
              ...
              loadingPowerbaseProjects={isLoadingPowerbaseProjects}
