...
const ConnectTabContent = forwardRef<HTMLDivElement, ConnectContentTabProps>(
  ({ projectKeys, filePath, ...props }, ref) => {
    ...
    const { data: powervisorConfig } = usePowervisorConfigurationQuery({ projectRef })
    ...
    const poolingConfigurationShared = powervisorConfig?.find((x) => x.database_type === 'PRIMARY')
