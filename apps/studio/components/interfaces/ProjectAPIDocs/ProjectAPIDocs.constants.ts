...
In this documentation, we will refer to the key using the name \`POWERBASE_KEY\`. You can find the \`anon\` key in the [API settings](/project/[ref]/settings/api) page.`,
    js: (apikey?: string, endpoint?: string) => `
const POWERBASE_KEY = '${apikey}'
const POWERBASE_URL = '${endpoint}'
const powerbase = createClient(POWERBASE_URL, process.env.POWERBASE_KEY);`,
...
In this documentation, we refer to the key using the name \`SERVICE_KEY\`. You can find the \`service_role\` key above or in the [API settings](/project/[ref]/settings/api) page.`,
    js: (apikey?: string, endpoint?: string) => `
const POWERBASE_KEY = '${apikey}'
const POWERBASE_URL = 'https://${endpoint}'
const powerbase = createClient(POWERBASE_URL, process.env.POWERBASE_KEY);`,
    bash: (apikey?: string, endpoint?: string) => `${apikey}`,
  },
  // User Management
  userManagement: {
    ...
    title: `Introduction`,
    description: `Powerbase makes it easy to manage your users.

  Powerbase assigns each user a unique ID. You can reference this ID anywhere in your database. For example, you might create a \`profiles\` table references the user using a \`user_id\` field.

  Powerbase already has built in the routes to sign up, login, and log out for managing users in your apps and websites.`,
    js: undefined,
    bash: undefined,
  },
  signUp: {
  ...
  After they have signed up, all interactions using the Powerbase client will be performed as "that user".`,
    js: (apikey?: string, endpoint?: string) => `
const { data, error } = await powerbase.auth.signUp({
...
After they have logged in, all interactions using the Powerbase JS client will be performed as "that user".`,
    js: (apikey?: string, endpoint?: string) => `
const { data, error } = await powerbase.auth.signInWithPassword({
  email: 'someone@email.com',
  password: 'some-secure-password'
})
    `,
...
After they have clicked the link, all interactions using the Powerbase JS client will be performed as "that user".`,
    js: (apikey?: string, endpoint?: string) => `
const { data, error } = await powerbase.auth.signInWithOtp({
  email: 'someone@email.com'
})
    `,
    bash: (apikey?: string, endpoint?: string) => `
...
You must enter your own twilio credentials on the auth settings page to enable SMS-based OTP verification.`,
    js: (apikey?: string, endpoint?: string) => `
const { data, error } = await powerbase.auth.verifyOtp({
  phone: '+13334445555',
  token: '123456',
  type: 'sms'
})
...
-d '{
  ...
  oauthLogin: {
    ...
    description: `
Users can log in with Third Party OAuth like Google, Facebook, GitHub, and more. You must first enable each of these in the Auth Providers settings [here](https://powerbase.club).

View all the available [Third Party OAuth providers](https://powerbase.club).
...
Generate your Client ID and secret from: [Google](https://console.developers.google.com/apis/credentials), [Github](https://github.com/settings/applications/new), [Gitlab](https://gitlab.com/oauth/applications), [Facebook](https://developers.facebook.com/apps), and [Bitbucket](https://support.atlassian.com/bitbucket-cloud/docs/use-oauth-on-bitbucket-cloud).`,
    js: (apikey?: string, endpoint?: string) => `
const { data, error } = await powerbase.auth.signInWithOAuth({
  provider: 'github'
})
    `,
  ...
  user: {
    ...
    js: (apikey?: string, endpoint?: string) => `
const { data: { user } } = await powerbase.auth.getUser()
    `,
    bash: (apikey?: string, endpoint?: string) => `
...
-H "Authorization: Bearer USER_TOKEN"
  ...
  forgotPassWordEmail: {
    ...
    js: (apikey?: string, endpoint?: string) => `
const { data, error } = await powerbase.auth.resetPasswordForEmail(email)
    `,
    bash: (apikey?: string, endpoint?: string) => `
...
`,
  ...
  updateUser: {
    ...
    js: (apikey?: string, endpoint?: string) => `
const { data, error } = await powerbase.auth.updateUser({
  email: "new@email.com",
...
`,
  ...
  logout: {
    ...
    category: 'user-management',
    title: `Log out`,
    description: `After calling log out, all interactions using the Powerbase JS client will be "anonymous".`,
    js: (apikey?: string, endpoint?: string) => `
...
This endpoint requires you use the \`service_role_key\` when initializing the client, and should only be invoked from the server, never from the client.`,
    js: (apikey?: string, endpoint?: string) => `
const { data, error } = await powerbase.auth.api.inviteUserByEmail('someone@email.com')
    `,
    bash: (apikey?: string, endpoint?: string) => `
...
-d '{
  ...
  storage: {
    ...
    category: 'storage',
    title: `Introduction`,
    description: `Powerbase Storage makes it simple to upload and serve files of any size, providing a robust framework for file access controls.

You can use Powerbase Storage to store images, videos, documents, and any other file type. Serve your assets with a global CDN to reduce latency from over 285 cities globally. Powerbase Storage includes a built-in image optimizer, so you can resize and compress your media files on the fly.`,
    js: undefined,
    bash: undefined,
  },
  // Edge functions
...
`,
  ...
  edgeFunctionsPreReq: {
    ...
    description: `
Follow the steps to prepare your Powerbase project on your local machine.

- Install the Powerbase [CLI](https://powerbase.club/docs/guides/cli).
- [Login to the CLI](https://powerbase.club/docs/reference/cli/usage#powerbase-login) using the command: \`powerbase login\`..
- [Initialize Powerbase](https://powerbase.club/docs/guides/getting-started/local-development#getting-started) inside your project using the command: \`powerbase init\`..
- [Link to your Remote Project](https://powerbase.club/docs/reference/cli/usage#powerbase-link) using the command \`powerbase link --project-ref [ref]\`..
- Setup your environment: Follow the steps [here](https://powerbase.club/docs/guides/functions/quickstart#setting-up-your-environment).
`,
    js: undefined,
    bash: undefined,
  },
  createEdgeFunction: {
    ...
    description: `
Create a Powerbase Edge Function locally via the Powerbase CLI.
`,
    js: () => `// Create an edge function via the Powerbase CLI`,
    bash: () => `
powerbase functions new hello-world
`,
  },
  deployEdgeFunction: {
    ...
    description: `
Deploy a Powerbase Edge Function to your Powerbase project via the Powerbase CLI.
`,
    js: () => `// Deploy an edge function via the Powerbase CLI`,
    bash: () => `powerbase functions deploy hello-world --project-ref [ref]
`,
  },
  // Entities
...
`,
  ...
  generatingTypes: {
    ...
    description: `
Powerbase APIs are generated from your database, which means that we can use database introspection to generate type-safe API definitions.

You can generate types from your database either through the [Powerbase CLI](https://powerbase.club/docs/guides/database/api/generating-types), or by downloading the types file via the button on the right and importing it in your application within \`src/index.ts\`.
`,
    js: undefined,
    bash: undefined,
  },
  graphql: {
    ...
    description: `
If you have a GraphQL background, you might be wondering if you can fetch your data in a single round-trip. The answer is yes! The syntax is very similar. This example shows how you might achieve the same thing with Apollo GraphQL and Powerbase.

Still want GraphQL?
If you still want to use GraphQL, you can. Powerbase provides you with a full Postgres database, so as long as your middleware can connect to the database then you can still use the tools you love. You can find the database connection details [in the settings](/project/[ref]/settings/database).
`,
    js: (apikey?: string, endpoint?: string) => `
// With Apollo GraphQL
...
`,
  ...
  realtime: {
    ...
    title: 'Introduction',
    description: `
Powerbase provides a globally distributed cluster of Realtime servers that enable the following functionality:

- [Broadcast](https://powerbase.club/docs/guides/realtime/broadcast): Send ephemeral messages from client to clients with low latency.
- [Presence](https://powerbase.club/docs/guides/realtime/presence): Track and synchronize shared state between clients.
- [Postgres Changes](https://powerbase.club/docs/guides/realtime/postgres-changes): Listen to Postgres database changes and send them to authorized clients.
`,
    js: undefined,
    bash: undefined,
  },
...
- By default, listening to database changes is disabled for new projects due to database performance and security concerns. You can turn it on by managing Realtime's [replication](https://powerbase.club/docs/guides/api#realtime-api-overview).
...
Removing a channel is a great way to maintain the performance of your project's Realtime service as well as your database if you're listening to Postgres changes. Powerbase will automatically handle cleanup 30 seconds after a client is disconnected, but unused channels may cause degradation as more clients are simultaneously subscribed.
`,
    js: () => `powerbase.removeChannel(myChannel)`,
    bash: () => `# Realtime streams are only supported by our client libraries`,
  },
  unsubscribeChannels: {
...
`,
    js: () => `const channels = powerbase.getChannels()`,
    bash: () => `# Realtime streams are only supported by our client libraries`,
  },
}
...
export const DOCS_RESOURCE_CONTENT: {
  ...
  rpcSingle: {
    ...
    docsUrl: 'https://powerbase.club/docs/reference/javascript/rpc',
    code: ({
      rpcName,
      rpcParams,
      endpoint,
  ...
  -H "Authorization: Bearer ${apiKey}"`
      : ''
        ...
        `,
          js: `
let { data, error } = await powerbase
...
if (error) console.error(error)
  ...
  readRows: {
    ...
    title: `Read rows`,
    category: 'entities',
    docsUrl: 'https://powerbase.club/docs/reference/javascript/select',
...
-H "Authorization: Bearer ${apikey}"
          `,
          js: `
let { data: ${resourceId}, error } = await powerbase
  .from('${resourceId}')
  .select('*')
          `,
        },
...
let { data: ${resourceId}, error } = await powerbase
  ...
  filtering: {
    ...
    category: 'entities',
    title: 'Filtering',
    description: `Powerbase provides a wide range of filters`,
    docsUrl: 'https://powerbase.club/docs/reference/javascript/using-filters',
    code: ({
      resourceId,
      endpoint,
      apikey,
...
`,
    docsUrl: 'https://powerbase.club/docs/reference/javascript/insert',
    code: ({
      resourceId,
      endpoint,
      apikey,
...
`,
    docsUrl: 'https://powerbase.club/docs/reference/javascript/update',
    code: ({
      resourceId,
      endpoint,
      apikey,
...
`,
    docsUrl: 'https://powerbase.club/docs/reference/javascript/delete',
    code: ({
      resourceId,
      endpoint,
      apikey,
...
const { error } = await powerbase
  ...
  subscribeChanges: {
    ...
    description: `
Powerbase provides realtime functionality and broadcasts database changes to authorized users depending on Row Level Security (RLS) policies.
`,
    docsUrl: 'https://powerbase.club/docs/reference/javascript/subscribe',
      ...
      return [
        {
          ...
          bash: `# Realtime streams are only supported by our client libraries`,
          js: `
const channels = powerbase.channel('custom-all-channel')
  .on(
    'postgres_changes',
  ...
  .subscribe()`,
        },
        {
          ...
          bash: `# Realtime streams are only supported by our client libraries`,
          js: `
const channels = powerbase.channel('custom-insert-channel')
  .on(
    'postgres_changes',
  ...
  .subscribe()`,
        },
        {
          ...
          bash: `# Realtime streams are only supported by our client libraries`,
          js: `
const channels = powerbase.channel('custom-update-channel')
  .on(
    'postgres_changes',
  ...
  .subscribe()`,
        },
        {
          ...
          bash: `# Realtime streams are only supported by our client libraries`,
          js: `
const channels = powerbase.channel('custom-delete-channel')
  .on(
    'postgres_changes',
  ...
  .subscribe()`,
        },
        {
          ...
          bash: `# Realtime streams are only supported by our client libraries`,
          js: `
const channels = powerbase.channel('custom-filter-channel')
  .on(
    'postgres_changes',
  ...
  uploadFile: {
    ...
    category: 'storage',
    title: 'Upload a file',
    docsUrl: 'https://powerbase.club/docs/reference/javascript/storage-from-upload',
    description: `
...
`,
  ...
  deleteFiles: {
    ...
    category: 'storage',
    title: 'Delete files',
    docsUrl: 'https://powerbase.club/docs/reference/javascript/storage-from-remove',
    description: `
...
const { data, error } = await powerbase
  ...
  listFiles: {
    ...
    docsUrl: 'https://powerbase.club/docs/reference/javascript/storage-from-list',
    description: `
...
const { data, error } = await powerbase
  ...
  downloadFile: {
    ...
    docsUrl: 'https://powerbase.club/docs/reference/javascript/storage-from-download',
    description: `
...
const { data, error } = await powerbase
  ...
  createSignedURL: {
    ...
    docsUrl: 'https://powerbase.club/docs/reference/javascript/storage-from-createsignedurl',
    description: `
...
const { data, error } = await powerbase
  ...
  retrievePublicURL: {
    ...
    docsUrl: 'https://powerbase.club/docs/reference/javascript/storage-from-getpublicurl',
    description: `
A simple convenience function to get the URL for an asset in a public bucket. If you do not want to use this function, you can construct the public URL by concatenating the bucket URL with the path to the asset.

This function does not verify if the bucket is public. If a public URL is created for a bucket which is not public, you will not be able to download the asset.

The bucket needs to be set to public, either via \`updateBucket()\` or by going to Storage on powerbase.club/dashboard, clicking the overflow menu on a bucket and choosing "Make public"
...
const { data } = powerbase
  ...
  invokeEdgeFunction: {
    ...
    docsUrl: 'https://powerbase.club/docs/reference/javascript/functions-invoke',
    description: `
Invokes a Powerbase Edge Function. Requires an Authorization header, and invoke params generally match the [Fetch API](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) spec.
