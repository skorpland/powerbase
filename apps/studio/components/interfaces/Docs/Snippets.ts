const snippets = {
  ...
  install: () => ({
    ...
    js: {
      language: 'bash',
      code: `npm install --save @skorpland/powerbase-js`,
    },
  }),
...
const powerbase = createClient(powerbaseUrl, powerbaseKey)`,
    ...
    python: {
      ...
      code: `
import os
from powerbase import create_client, Client
url: str = '${endpoint}'
key: str = os.environ.get("POWERBASE_KEY")
powerbase: Client = create_client(url, key)
`,
    },
    dart: {
      language: 'dart',
      code: `
const powerbaseUrl = '${endpoint}';
const powerbaseKey = String.fromEnvironment('POWERBASE_KEY');
Future<void> main() async {
  await Powerbase.initialize(url: powerbaseUrl, anonKey: powerbaseKey);
  runApp(MyApp());
}`,
    },
  }),
...
const powerbase = createClient(POWERBASE_URL, process.env.${keyName || 'POWERBASE_KEY'});
`,
    },
  }),
  rpcSingle: ({
...
`,
  ...
  subscribeAll: (listenerName: string, resourceId: string) => ({
    ...
    js: {
      language: 'js',
      code: `
const ${listenerName} = powerbase.channel('custom-all-channel')
  .on(
    'postgres_changes',
  ...
  subscribeInserts: (listenerName: string, resourceId: string) => ({
    ...
    js: {
      language: 'js',
      code: `
const ${listenerName} = powerbase.channel('custom-insert-channel')
  .on(
    'postgres_changes',
  ...
  subscribeUpdates: (listenerName: string, resourceId: string) => ({
    ...
    js: {
      language: 'js',
      code: `
const ${listenerName} = powerbase.channel('custom-update-channel')
  .on(
    'postgres_changes',
  ...
  subscribeDeletes: (listenerName: string, resourceId: string) => ({
    ...
    js: {
      language: 'js',
      code: `
const ${listenerName} = powerbase.channel('custom-delete-channel')
  .on(
    'postgres_changes',
  ...
  subscribeEq: (listenerName: string, resourceId: string, columnName: string, value: string) => ({
    ...
    js: {
      language: 'js',
      code: `
const ${listenerName} = powerbase.channel('custom-filter-channel')
  .on(
    'postgres_changes',
...
`,
    ...
    js: {
      language: 'js',
      code: `
let { data, error } = await powerbase.auth.signUp({
  email: 'someone@email.com',
  password: '${randomPassword}'
})
`,
...
`,
    ...
    js: {
      language: 'js',
      code: `
let { data, error } = await powerbase.auth.signInWithPassword({
  email: 'someone@email.com',
  password: '${randomPassword}'
})
`,
...
`,
    ...
    js: {
      language: 'js',
      code: `
let { data, error } = await powerbase.auth.signInWithOtp({
  email: 'someone@email.com'
})
`,
    },
...
`,
    ...
    js: {
      language: 'js',
      code: `
let { data, error } = await powerbase.auth.verifyOtp({
  phone: '+13334445555',
  token: '123456',
  type: 'sms'
})
...
`,
    ...
    js: {
      language: 'js',
      code: `
let { data, error } = await powerbase.auth.admin.inviteUserByEmail('someone@email.com')
`,
    },
  }),
...
`,
    ...
    js: {
      language: 'js',
      code: `
let { data, error } = await powerbase.auth.signInWithOAuth({
  provider: 'github'
})
`,
    },
...
`,
    ...
    js: {
      language: 'js',
      code: `
let { data, error } = await powerbase.auth.resetPasswordForEmail(email)
`,
    },
  }),
...
`,
    ...
    js: {
      language: 'js',
      code: `
let { error } = await powerbase.auth.signOut()
`,
    },
  }),
}
