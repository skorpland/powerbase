...
export default function GeneratingTypes({ selectedLang }: Props) {
  ...
  const onClickGenerateTypes = async () => {
    try {
      ...
      element.setAttribute('download', 'powerbase.ts')
  ...
  return (
    <>
      <h2 className="doc-heading flex items-center justify-between">
        ...
        <DocsButton href="https://powerbase.club/docs/guides/database/api/generating-types" />
      </h2>
      <div className="doc-section">
        <article className="code-column text-foreground">
          <p>
            Powerbase APIs are generated from your database, which means that we can use database
            introspection to generate type-safe API definitions.
          </p>
          <p>
            You can generate types from your database either through the{' '}
            <Link href="https://powerbase.club/docs/guides/database/api/generating-types">
              Powerbase CLI
            </Link>
...
`,
  ...
  generateTypes: (ref: string) => ({
    ...
    bash: {
      code: `
npx powerbase gen types typescript --project-id "${ref}" --schema public > types/powerbase.ts
`,
    },
  }),
}
