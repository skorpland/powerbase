...
const GitHubIntegrationConnectionForm = ({
  ...
  const FormSchema = z.object({
    powerbaseDirectory: z
      .string()
      .default(connection.metadata?.powerbaseConfig?.powerbaseDirectory ?? ''),
    powerbaseChangesOnly: z
      .boolean()
      .default(connection.metadata?.powerbaseConfig?.powerbaseChangesOnly ?? false),
    branchLimit: z.string().default(String(connection.metadata?.powerbaseConfig?.branchLimit ?? 50)),
  })
  const form = useForm<z.infer<typeof FormSchema>>({
    resolver: zodResolver(FormSchema),
    defaultValues: {
      powerbaseDirectory: connection?.metadata?.powerbaseConfig?.powerbaseDirectory,
      powerbaseChangesOnly: connection?.metadata?.powerbaseConfig?.powerbaseChangesOnly,
      branchLimit: String(connection?.metadata?.powerbaseConfig?.branchLimit),
    },
  })
  ...
  function onSubmit(data: z.infer<typeof FormSchema>) {
    ...
    updateConnection({
      ...
      workdir: data.powerbaseDirectory,
      powerbaseChangesOnly: data.powerbaseChangesOnly,
    ...
    <div className="flex flex-col gap-6 px-6 py-4">
      ...
      <Form_Shadcn_ {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-6">
                  ...
                  <div className="relative">
                    ...
                    <RotateCcw
                      className={cn(
                        ...
                        field.value !== String(connection.metadata?.powerbaseConfig?.branchLimit)
          ...
          <FormField_Shadcn_
            control={form.control}
            name="powerbaseDirectory"
            render={({ field }) => (
              <FormItem_Shadcn_ className="flex flex-col">
                <FormLabel_Shadcn_ className="!text">Powerbase directory</FormLabel_Shadcn_>
                <FormDescription_Shadcn_ className="text-xs text-foreground-lighter !mt-0 !mb-1">
                  Path in your repository where <code>powerbase</code> directory for this connection
                  lives.
                </FormDescription_Shadcn_>
                <FormControl_Shadcn_ className="flex gap-3 items-center">
                  <div className="relative">
                    ...
                    <RotateCcw
                      className={cn(
                        ...
                        field.value !== connection.metadata?.powerbaseConfig?.powerbaseDirectory
                          ? 'opacity-100 transition'
                          : 'opacity-0'
                      )}
                      onClick={() => form.reset()}
          ...
          <FormField_Shadcn_
            ...
            render={({ field }) => (
              <FormItem_Shadcn_ className="space-y-0 flex gap-x-4">
                ...
                <div>
                  <FormLabel_Shadcn_ className="!text">Powerbase changes only</FormLabel_Shadcn_>
                  <FormDescription_Shadcn_ className="text-xs text-foreground-lighter">
                    Trigger branch creation only when there were changes to <code>powerbase</code>{' '}
                    directory.
                  </FormDescription_Shadcn_>
                </div>
              </FormItem_Shadcn_>
