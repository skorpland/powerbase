...
export const LOGS_EXPLORER_DOCS_URL =
  'https://powerbase.club/docs/guides/platform/logs#querying-with-the-logs-explorer'
...
group by
  ...
  {
    label: 'Metadata IP',
    description: 'List all IP addresses that used the Powerbase API',
    mode: 'custom',
    searchString: `select
...
`,
  ...
  },
  {
    ...
    description: 'List all ISO 3166-1 alpha-2 country codes that used the Powerbase API',
    mode: 'custom',
    searchString: `select
  cf.country,
  count(*) as count
...
`,
  ...
  },
  {
    ...
    mode: 'custom',
    description: 'List all Powerbase API requests that are slow',
    searchString: `select
  cast(timestamp as datetime) as timestamp,
  event_message,
  r.origin_time
...
`,
  ...
  },
  {
    ...
    description: 'List all Powerbase API requests that responded witha 5XX status code',
    mode: 'custom',
    searchString: `select
  cast(timestamp as datetime) as timestamp,
  event_message,
...
`,
  ...
  {
    label: 'Top Paths',
    description: 'List the most requested Powerbase API paths',
    mode: 'custom',
    searchString: `select
  r.path as path,
  r.search as params,
...
export const LOGS_TABLES = {
  ...
  powervisor: LogsTableName.POWERVISOR,
...
export const LOGS_SOURCE_DESCRIPTION = {
  ...
  [LogsTableName.POWERVISOR]: 'Shared connection pooler logs for PostgreSQL',
