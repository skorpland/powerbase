...
export const uploadAttachments = async (ref: string, files: File[]) => {
  const supportPowerbaseClient = createClient(SUPPORT_API_URL, SUPPORT_API_KEY, {
    auth: {
  ...
  const uploadedFiles = await Promise.all(
    filesToUpload.map(async (file) => {
      ...
      const { data, error } = await supportPowerbaseClient.storage
...
export function generateLink(pageType: PageType, link: string): string {
  switch (pageType) {
    case PageType.Markdown:
    case PageType.Reference:
      return `https://powerbase.club/docs${link}`
    case PageType.Integration:
      return `https://powerbase.club${link}`
    case PageType.GithubDiscussion:
      return link
    default:
