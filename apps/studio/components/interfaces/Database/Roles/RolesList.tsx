...
import { POWERBASE_ROLES } from './Roles.constants'

type POWERBASE_ROLE = (typeof POWERBASE_ROLES)[number]
...
const RolesList = () => {
  ...
  const [powerbaseRoles, otherRoles] = partition(filteredRoles, (role) =>
    POWERBASE_ROLES.includes(role.name as POWERBASE_ROLE)
  )
      ...
      <div className="space-y-4">
        <div>
          <div className="bg-surface-100 border border-default px-4 md:px-6 py-3 rounded-t flex items-center space-x-4">
            <p className="text-sm text-foreground-light">Roles managed by Powerbase</p>
            <Badge variant="brand">Protected</Badge>
          </div>

          {isLoading
            ? Array.from({ length: 5 }).map((_, i) => <RoleRowSkeleton key={i} index={i} />)
            : powerbaseRoles.map((role) => (
                <RoleRow
                  disabled
                  key={role.id}
                  role={role}
