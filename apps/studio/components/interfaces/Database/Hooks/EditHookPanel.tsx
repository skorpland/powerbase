import type { PostgresTable, PostgresTrigger } from '@skorpland/postgres-meta'
...
const EditHookPanel = ({ visible, selectedHook, onClose }: EditHookPanelProps) => {
  ...
  const isEdgeFunction = (url: string) =>
    url.includes(`https://${ref}.functions.powerbase.${restUrlTld}/`) ||
    url.includes(`https://${ref}.powerbase.${restUrlTld}/functions/`)
  ...
  const validate = (values: any) => {
    ...
    } else if (values.function_type === 'powerbase_function') {
      // For Powerbase Edge Functions
  ...
  const onSubmit = async (values: any) => {
    ...
    const payload: any = {
      ...
      function_schema: 'powerbase_functions',
      function_args: [
        values.http_url,
        values.http_method,
...
const FormContents = ({
  ...
  useEffect(() => {
    if (values.function_type === 'http_request') {
      ...
      const defaultFunctionUrl = `https://${projectRef}.powerbase.${restUrlTld}/functions/v1/${fnSlug}`
      const updatedValues = {
        ...values,
