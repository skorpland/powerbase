...
-- you can call the function from your browser with powerbase-js
-- const { data, error } = await powerbase.rpc('increment', { row_id: 2 })
  `.trim(),
  },
  {
    id: 10,
...
`.trim(),
  },
  {
    ...
    sql: `
--
-- For use with https://github.com/skorpland/powerbase/tree/master/examples/slack-clone/nextjs-slack-clone
...
comment on column public.users.id is 'References the internal Powerbase Auth user.';
...
as $$
  ...
  begin
    ...
    if position('+poweradmin@' in new.email) > 0 then
      insert into public.user_roles (user_id, role) values (new.id, 'admin');
    elsif position('+powermod@' in new.email) > 0 then
...
alter publication powerbase_realtime add table public.channels;
alter publication powerbase_realtime add table public.messages;
alter publication powerbase_realtime add table public.users;

-- DUMMY DATA
insert into public.users (id, username)
values
    ('8d0fd2b3-9ca7-4d9e-a95f-9e13dded323e', 'powerbot');
...
--
-- For use with:
-- https://github.com/skorpland/powerbase/tree/master/examples/todo-list/sveltejs-todo-list or
-- https://github.com/skorpland/examples-archive/tree/main/powerbase-js-v1/todo-list
--
...
`.trim(),
  },
  {
    ...
    description: 'Sets up a the Schema and Tables for the NextAuth Powerbase Adapter.',
    sql: `
--
...
select pgtle.uninstall_extension_if_exists('powerbase-dbdev');
drop extension if exists "powerbase-dbdev";
select
...
from http(
    (
        ...
        || '&package_name=eq.powerbase-dbdev'
        || '&order=version.desc'
        || '&limit=1',
        array[
...
create extension "powerbase-dbdev";
select dbdev.install('powerbase-dbdev');
drop extension if exists "powerbase-dbdev";
create extension "powerbase-dbdev";
`.trim(),
  },
...
as $$
  ...
  begin
    ...
    -- finally let Powerbase Auth do the default behavior for a failed attempt
    return jsonb_build_object('decision', 'continue');
  end;
$$;
...
grant usage on schema public to powerbase_auth_admin;`.trim(),
  },
  {
    id: 27,
    type: 'template',
...
grant execute on function public.custom_access_token_hook to powerbase_auth_admin;
