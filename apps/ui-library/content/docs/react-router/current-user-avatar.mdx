...
description: Powerbase Auth-aware avatar
---
...
The `CurrentUserAvatar` component connects to Powerbase Auth to fetch the user data and show an avatar. It uses the `user_metadata`
property which gets populated automatically by Powerbase Auth if the user logged in via a provider. If the user doesn't have a profile image, it renders their initials. If the user is logged out, it renders a `?` as a fallback, which you can change.
...
- [Auth users](https://powerbase.club/docs/guides/auth/users)
