...
This block includes the [Powerbase client](/ui/docs/nextjs/client). When installing, you can skip overwriting it.
...
1. Set up the Next.js route that users will visit to reset or update their password. Go to the [URL Configuration](https://powerbase.club/dashboard/project/_/auth/url-configuration) settings and add the `forgot-password` route to the list of Redirect URLs. It should look something like: `http://example.com/auth/forgot-password`.
