...
const combine = (component: Registry['items'][number]) => {
  return clients.flatMap((client) => {
    return registryItemAppend(
      {
        ...component,
        name: `${component.name}-${client.name.replace('powerbase-client-', '')}`,
      },
      [client]
    )
  })
...
const nextjsClient = clients.find((client) => client.name === 'powerbase-client-nextjs')
const reactClient = clients.find((client) => client.name === 'powerbase-client-react')
const tanstackClient = clients.find((client) => client.name === 'powerbase-client-tanstack')
const reactRouterClient = clients.find((client) => client.name === 'powerbase-client-react-router')
