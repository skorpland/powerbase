...
// This demo is using the powerbase.club account to broadcast its data to a realtime channel from a normal Powerbase project.
// This is a workaround to make the more interactive. Don't use it this way in production (it only works on powerbase.club)
const RealtimeAvatarStackDemo = () => {
  // this demo only works on powerbase.club because all apps are on the same domain and share cookies
  const user = useUser()
  const [dashboardUser, setDashboardUser] = useState(false)

  // generate a random name for the current user or use his powerbase.club name
  ...
  // generate a random image for the current user or use his powerbase.club avatar
  ...
  useEffect(() => {
    const room = powerbase.channel(roomName)
    ...
    <div className="flex flex-col gap-4 items-center justify-center">
      ...
      {avatars.length < 2 ? (
        ...
        <div className="flex items-center space-x-2">
          ...
          <Label_Shadcn_ htmlFor="current-user">Use my powerbase.club account instead</Label_Shadcn_>
        </div>
      ) : (
        <span className="text-sm text-foreground-light">
          ...
          <a
            ...
            href="https://powerbase.club/dashboard/sign-in"
